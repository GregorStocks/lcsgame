...
void mode_base()
{
  ...
  while (true)
  {
    ...
    if (activesquad)
      if (activesquad->squad[0]->location != -1)
    ...
    if (!forcewait)
    {
      ...
      if (len(squad) > 1 || (!activesquad && len(squad)))
        set_color(COLOR_WHITE, COLOR_BLACK, 0);
      else
        set_color(COLOR_BLACK, COLOR_BLACK, 1);
      ...
      set_color(COLOR_WHITE, COLOR_BLACK, 1);
      if (haveflag)
        mvaddstr(17, 39 - ((len(slogan) - 1) >> 1), slogan);
      else
    ...
    case 'o':
      ...
      break;
    case 'c':
      if (partysize) activesquad->activity.type = ACTIVITY_NONE;
      break;
    case 'a':
      activate();
      break;
