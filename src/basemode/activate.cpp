...
void activate(Creature *cr)
{
  ...
  while (true)
  {
    ...
    if (hostagecount > 0)
      set_color(COLOR_WHITE, COLOR_BLACK, state == 'i');
    else
    ...
    if (clinictime(*cr))
      set_color(COLOR_WHITE, COLOR_BLACK, state == 'm');
    else
      set_color(COLOR_BLACK, COLOR_BLACK, 1);
    ...
    if (havedead)
      set_color(COLOR_WHITE, COLOR_BLACK, state == 'z');
    else
      set_color(COLOR_BLACK, COLOR_BLACK, 1);
    ...
    if (activesquad)
      if (activesquad->squad[0]->location != -1) siege = &location[activesquad->squad[0]->location]->siege;
    char sieged = 0;
    ...
    if ((c >= 'a' && c <= 'z') || (c >= '0' && c <= '9'))
    {
      ...
      case 'm':
        ...
        break;
      case 'h':
        if (cr->get_skill(SKILL_FIRSTAID))
...
void select_makeclothing(Creature *cr)
{
  ...
  for (int a = 0; a < len(armortype); a++)
  {
    ...
    if (armortype[a]->deathsquad_legality() && (law[LAW_POLICEBEHAVIOR] != -2 || law[LAW_DEATHPENALTY] != -2))
      continue;
