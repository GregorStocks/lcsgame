    ...
    if (defense == 0 || defense == 3 || defense == 4) {
      if (autoconvict) {
        ...
        if (defense == 0)
          ...
          // Sleeper attorney
          defensepower = LCSrandom(71) + sleeperlawyer->get_skill(SKILL_LAW) * 2 + sleeperlawyer->get_skill(SKILL_PERSUASION) * 2;
...
void penalize(Creature &g, char lenient) {
  ...
  for (int l = 0; l < LAWFLAGNUM; l++)
    if (g.crimes_suspected[l] > 10) g.crimes_suspected[l] = 10;
...
void reeducation(Creature &g) {
  ...
  if (!g.attribute_check(ATTRIBUTE_HEART, DIFFICULTY_FORMIDABLE)) {
    if (g.juice > 0 && LCSrandom(2)) {
      ...
      addjuice(g, -50, 0);
    } else if (LCSrandom(15) > g.get_attribute(ATTRIBUTE_WISDOM, true) || g.get_attribute(ATTRIBUTE_WISDOM, true) < g.get_attribute(ATTRIBUTE_HEART, true)) {
      addstr(g.name, gamelog);
...
void prisonscene(Creature &g) {
  ...
  if (escaped == 0) {
    if (g.attribute_check(ATTRIBUTE_HEART, DIFFICULTY_HARD)) {
      effect = 1;
      if (LCSrandom(2) > 0)
        experience = pickrandom(good_experiences);
      else
      ...
      effect = -1;
      if (LCSrandom(2) > 0)
        experience = pickrandom(bad_experiences);
      else
        experience = pickrandom(general_experiences);
    }
  }
