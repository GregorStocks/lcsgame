...
int getsimplevoter(int leaning) { // no need for this to deal with Stalinism, this function deliberately only deals with the liberal vs. conservative spectrum
  int vote = leaning - 1;
  for (int i = 0; i < 2; i++)
...
void promoteVP() {
  ...
  switch (exec[EXEC_PRESIDENT]) { // make sure presparty is set correctly (if old President was moderate they could've been in Liberal OR Conservative Party,
  ...
  case ALIGN_CONSERVATIVE:
    ...
    break; // Conservatives and Arch-Conservatives align with the Conservative Party
  case ALIGN_MODERATE:
    break; // same party as previous President (either Liberal Party or Conservative Party)
  case ALIGN_LIBERAL:
  case ALIGN_ELITELIBERAL:
    presparty = LIBERAL_PARTY;
    break; // Liberals and Elite Liberals align with the Liberal Party
  case ALIGN_STALINIST:
    presparty = STALINIST_PARTY;
    break; // Stalinists align with the Stalinist Party
  }
  fillCabinetPost(EXEC_VP);            // the vacant VP slot needs to be filled
  if (abs(exec[EXEC_PRESIDENT]) > 1) { // new President has extreme views... make sure all cabinet members agree with new President
  ...
  } else { // new President has fairly moderate views, but anyone who differs too much from them must be replaced
...
void fillCabinetPost(int position) {
  ...
  else if (exec[EXEC_PRESIDENT] == ALIGN_STALINIST)
    exec[position] = ALIGN_STALINIST;
  else
...
void elections(char clearformess, char canseethings) {
  ...
  if (year % 4 == 0) {
    if (canseethings) {
      ...
      move(2, 0);
      if (stalinmode)
        addstr("After a long primary campaign, the people have rallied around three leaders...");
      else
    ...
    if (consvotes[0] > consvotes[1] && consvotes[0] > consvotes[2])
      candidate[CONSERVATIVE_PARTY][0] = ALIGN_ARCHCONSERVATIVE;
    else if (consvotes[1] > consvotes[2])
      candidate[CONSERVATIVE_PARTY][0] = ALIGN_CONSERVATIVE;
    else
      candidate[CONSERVATIVE_PARTY][0] = ALIGN_MODERATE;

    // determine liberal winner
    if (libvotes[0] > libvotes[1] && libvotes[0] > libvotes[2])
      candidate[LIBERAL_PARTY][0] = ALIGN_MODERATE;
    else if (libvotes[1] > libvotes[2])
      candidate[LIBERAL_PARTY][0] = ALIGN_LIBERAL;
    else
      candidate[LIBERAL_PARTY][0] = ALIGN_ELITELIBERAL;
    ...
    // name the candidates
    if (candidate[CONSERVATIVE_PARTY][0] == -2)
      generate_name(candidate[CONSERVATIVE_PARTY] + 1, GENDER_WHITEMALEPATRIARCH);
    else if (candidate[CONSERVATIVE_PARTY][0] == -1)
      generate_name(candidate[CONSERVATIVE_PARTY] + 1, GENDER_MALE);
    else
      generate_name(candidate[CONSERVATIVE_PARTY] + 1);
    ...
    {
      if (l % 2 == 0 && LCSrandom(5)) {                                            // Partyline Liberals (~40%) - except when stalin mode is enabled they can go Stalinist sometimes
      ...
      } else if (l % 2 == 1 && LCSrandom(5)) {                                      // Partyline Conservatives (~40%) - except when stalin mode is enabled they can go Stalinist sometimes
      ...
      if (l % 5 == 4) {
        ...
        vector<int> eligible;
        for (c = 0; c < 2 + stalinmode; c++)
          if (votes[c] == maxvote) eligible.push_back(c);
  ...
  if (canseethings) {
    for (p = 0; p < pnum; p++) {
      int propnum = 0;
      do {           // this loop prevents duplicate proposition numbers from occurring
        switch (p) { // 7 entries, as 7 is maximum value for pnum
        case 0:
        ...
        case 5:
          ...
          break;
        case 6:
          propnum = 17 * (17 - LCSrandom(2) * 6) * (2 - LCSrandom(2) * 1);
          break;
        }
  ...
  for (p = 0; p < pnum; p++) {
    ...
    canlaw.clear();
    for (l = 0; l < LAWNUM; l++)
      if (lawpriority[l] == maxprior && !lawtaken[l]) canlaw.push_back(l);
    ...
    if (canseethings) {
      ...
      case LAW_ABORTION:
        if (propdir[p] == 1)
          addstr("Strengthen Reproductive Freedom"); // was "Strengthen Abortion Rights"
        else
      ...
      case LAW_ANIMALRESEARCH:
        if (propdir[p] == 1)
          ...
          addstr("Help Scientists Cure Diseases"); // was "Expand Animal Research"
        break;
      case LAW_POLICEBEHAVIOR:
        if (propdir[p] == 1)
          addstr("Curtail Police Misconduct");
        else
          addstr("Help Police Fight Crime"); // was "Stop Harassment of Police Officers"
        break;
      case LAW_PRIVACY:
        if (propdir[p] == 1)
          addstr("Enhance Privacy Protection");
        else
          addstr("Promote Transparency and Accountability"); // was "Allow Corporations Access to Information"
        break;
      case LAW_DEATHPENALTY:
        if (propdir[p] == 1)
          addstr("Stop Barbaric Executions"); // was "Limit the Death Penalty"
        else
          addstr("Deter Criminals via Capital Punishment"); // "Expand Capital Punishment"
        break;
      case LAW_NUCLEARPOWER:
        if (propdir[p] == 1)
          addstr("Prevent Nuclear Meltdowns"); // was "Limit Nuclear Power"
        else
          addstr("Promote Alternative Energy Sources"); // was "Expand Nuclear Power"
        break;
      case LAW_POLLUTION:
        if (propdir[p] == 1)
          addstr("Protect our Environment"); // was "Punish Polluters"
        else
          addstr("Support American Manufacturing"); // was "Reward Industry"
        break;
      case LAW_LABOR:
        if (propdir[p] == 1)
          addstr("Protect Workers' Rights"); // was "Enhance Labor Standards"
        else
          addstr("Fight Corrupt Union Thugs"); // was "End Undue Union Influence"
        break;
      case LAW_GAY:
        if (propdir[p] == 1)
          addstr("Fight Homophobic Bigotry in our Laws"); // was "Expand Homosexual Rights"
        else
      ...
      case LAW_CORPORATE:
        if (propdir[p] == 1)
          ...
          addstr("Help Small Businesses Expand"); // was "Reward Small Businesses"
        break;
      case LAW_FREESPEECH:
        if (propdir[p] == 1)
          addstr("Protect Free Speech");
        else
          addstr("Limit Hate Speech"); // was "Limit Hurtful Speech"
        break;
      case LAW_TAX:
        if (propdir[p] == 1)
          addstr("Fight Income Inequality"); // was "Punish the Wealthy"
        else
          addstr("Cut Job-Killing Taxes"); // was "Stimulate Economic Growth"
        break;
      ...
      case LAW_GUNCONTROL:
        if (propdir[p] == 1)
          addstr("Prevent Mass Shootings"); // was "Prevent Gun Violence"
        else
          addstr("Protect our Second Amendment Rights"); // was "Assert our Second Amendment Rights"
        break;
      case LAW_WOMEN:
        if (propdir[p] == 1)
          addstr("Promote Gender Equality"); // was "Expand Women's Rights"
        else
          addstr("Preserve Traditional Gender Roles");
        break;
      case LAW_CIVILRIGHTS:
        if (propdir[p] == 1)
          addstr("Promote Racial Equality"); // was "Expand Civil Rights"
        else
          addstr("Fight Reverse Discrimination");
        break;
      case LAW_DRUGS:
        if (propdir[p] == 1)
          addstr("Repeal Oppressive Drug Laws"); // was "Limit Oppressive Drug Laws"
        else
          addstr("Fight Drug Trafficking"); // was "Strengthen the War On Drugs"
        break;
      ...
      case LAW_ELECTIONS:
        if (propdir[p] == 1)
          ...
          addstr("Limit Regulation of Political Speech"); // was "Limit Campaign Finance Reforms"
        break;
      case LAW_MILITARY:
        if (propdir[p] == 1)
          addstr("Shrink the Military-Industrial Complex"); // was "Limit Military Spending"
        else
          addstr("Support our Troops in Defending America"); // was "Strengthen our National Defense"
        break;
      case LAW_TORTURE:
        if (propdir[p] == 1)
          addstr("Protect Human Rights from Torture"); // was "Ban Torture Techniques"
        else
          addstr("Enhance Interrogations to Fight Terrorism"); // was "Permit Strong Tactics in Interrogations"
        break;
      case LAW_PRISONS:
        if (propdir[p] == 1) {
    ...
    for (int l = 0; l < 1000; l++) {
      ...
      if (l == 999) {
        if (yesvotes > 500)
          ...
          yeswin = (LCSrandom(100) < mood ? propdir[p] == 1 : propdir[p] == -1), recount = true;
      }

      if (canseethings && (l % 10 == 9)) {
        if ((l != 999 && yesvotes > l / 2) || (l == 999 && yeswin))
          set_color(COLOR_WHITE, COLOR_BLACK, 1);
...
void elections_senate(int senmod, char canseethings) {
  ...
  if (canseethings) {
    ...
    } else if (change[4] > 0 && change[0] <= 0 && change[5] <= 0) // L+ increased and C+ and Stalinist both same or decreased
    {
      ...
      winner = ALIGN_MODERATE; // nobody won
    switch (winner) {
    case ALIGN_ARCHCONSERVATIVE:
      addstr("The $$ U.S.A. Flag Eagle $$ Conservative Tea Party claims victory!");
      break;
    case ALIGN_CONSERVATIVE:
    ...
    case ALIGN_LIBERAL:
      ...
      break;
    case ALIGN_ELITELIBERAL:
      addstr("The Progressive Elite Social Liberal Green Party claims victory!");
      break;
    case ALIGN_STALINIST:
      addstr("The Stalinist Party claims victory!");
      break;
    }
  ...
  for (h = 0; h < HOUSENUM; h++) {
    ...
    if (termlimits)
      ...
      do {
        ...
        if (first) switch (law[LAW_ELECTIONS]) {
          case -2:
            if (LCSrandom(3)) vote2 = house[h];
            break; // 2/3 chance of incumbent winning no matter what (huge   advantage)
          case -1:
            if (LCSrandom(2)) vote2 = house[h];
            break; // 1/2 chance of incumbent winning no matter what (big    advantage)
          case 0:
            if (!LCSrandom(3)) vote2 = house[h];
            break; // 1/3 chance of incumbent winning no matter what (medium advantage)
          case 1:
            if (!LCSrandom(5)) vote2 = house[h];
            break; // 1/5 chance of incumbent winning no matter what (small  advantage)
          case 2:
            if (!LCSrandom(8)) vote2 = house[h];
            break; // 1/8 chance of incumbent winning no matter what (tiny   advantage)
          }
        first = false;
...
void supremecourt(char clearformess, char canseethings) {
  ...
  for (c = 0; c < cnum; c++) {
    ...
    for (int l = 0; l < COURTNUM; l++) {
      ...
      if (canseethings) {
        if (l == COURTNUM - 1 && yeswin)
          set_color(COLOR_WHITE, COLOR_BLACK, 1);
        else if (l == COURTNUM - 1)
          set_color(COLOR_BLACK, COLOR_BLACK, 1);
        else
          set_color(COLOR_WHITE, COLOR_BLACK, 0);
        move(c * 3 + 2, 63);
  ...
  if (LCSrandom(10) >= 6) {
    ...
    float sen = 0;
    for (int s = 0; s < 100; s++)
      sen += (senate[s] == ALIGN_STALINIST ? -3 : senate[s]);
    sen /= 100.0f;

    float consensus = (president + sen) * .5f;

    if (consensus < -2.1f)
      court[j] = ALIGN_STALINIST;
    else if (consensus < -1.5f)
      court[j] = ALIGN_ARCHCONSERVATIVE;
    else if (consensus < -.5f)
      court[j] = ALIGN_CONSERVATIVE;
    else if (consensus < .5f)
      court[j] = ALIGN_MODERATE;
    else if (consensus < 1.5f)
      court[j] = ALIGN_LIBERAL;
    else
...
void congress(char clearformess, char canseethings) {
  ...
  for (l = 0; l < LAWNUM; l++) {
    ...
    for (int cl = 0; cl < HOUSENUM; cl++) {
      ...
      if (housealign == ALIGN_STALINIST) {
        if (stalinview(l, true))
          housealign = ALIGN_ELITELIBERAL;
        else
          housealign = ALIGN_ARCHCONSERVATIVE;
      }
      if (law[l] < housealign)
        pup++;
      else if (law[l] > housealign)
        pdown++;
    ...
    for (int sl = 0; sl < SENATENUM; sl++) {
      ...
      if (senatealign == ALIGN_STALINIST) {
        if (stalinview(l, true))
          senatealign = ALIGN_ELITELIBERAL;
        else
          senatealign = ALIGN_ARCHCONSERVATIVE;
      }
      if (law[l] < senatealign)
        pup += 4;
      else if (law[l] > senatealign)
        pdown += 4;
    ...
    for (int i = 0; i < 4; i++)
      if (10 + 20 * i < mood) public_position++;
  ...
  for (c = 0; c < cnum; c++) {
    ...
    for (int l = 0; l < HOUSENUM; l++) {
      ...
      if (l == HOUSENUM - 1) {
        ...
        if (yesvotes_s == SENATEMAJORITY - 1) { //TIE BREAKER
          int vote;
          if ((exec[EXEC_VP] >= -1 && exec[EXEC_VP] <= 1) ||
            ...
            vote = (exec[EXEC_PRESIDENT] + // only consult Cabinet and random number generator if alignment between -1 and 1
                    ...
                    exec[EXEC_STATE] +
                    exec[EXEC_ATTORNEY] + LCSrandom(9) - 4) /
                   4;
          else {
            vote = exec[EXEC_VP];
            if (vote == ALIGN_STALINIST) {
  ...
  int havebill = 0;
  for (c = 0; c < cnum; c++)
    if (killbill[c] != BILL_FAILED) havebill++;
  ...
  for (int j = 0; j < COURTNUM; j++)
    if (court[j] != ALIGN_ELITELIBERAL) tossj = true;
...
int publicmood(int l) {
  ...
  case LAW_CORPORATE:
    return (attitude[VIEW_CORPORATECULTURE] + attitude[VIEW_CEOSALARY]) / 2; // <-- We'll be merging these two views here because there is no CEO salary law.
  case LAW_FREESPEECH:
...
bool stalinview(short view, bool islaw) {
  if (islaw) switch (view) {
    case LAW_STALIN:
      return false; // Liberals and Stalinists don't get along
    case LAW_MOOD:
      return false; // Liberals and Stalinists don't get along
    case LAW_ABORTION:
      return true; // Stalinists agree that abortion is good, although technically they don't let women choose
    case LAW_ANIMALRESEARCH:
      return false; // Stalinists are in favor of unethical research
    case LAW_POLICEBEHAVIOR:
      return false; // Stalinists use police for brutal repression
    case LAW_PRIVACY:
      return false; // Stalinists don't believe in privacy
    case LAW_DEATHPENALTY:
      return false; // Stalinists execute lots of people
    case LAW_NUCLEARPOWER:
      return false; // Stalinists believe the more nuclear, the better
    case LAW_POLLUTION:
      return false; // Stalinists don't care about pollution
    case LAW_LABOR:
      return true; // Stalinists say, Workers of the world unite!
    case LAW_GAY:
      return false; // Stalinists discriminate against gay people
    case LAW_CORPORATE:
      return true; // Stalinists hate rich people and corporations
    case LAW_FREESPEECH:
      return false; // Stalinists don't allow any dissent
    case LAW_FLAGBURNING:
      return true; // Stalinists regularly burn flags
    case LAW_GUNCONTROL:
      return true; // Stalinists don't want any armed resistance
    case LAW_TAX:
      return true; // Stalinists support communist income redistribution
    case LAW_WOMEN:
      return false; // Stalinists discriminate against women
    case LAW_CIVILRIGHTS:
      return false; // Stalinists discriminate against ethnic groups
    case LAW_DRUGS:
      return false; // Stalinists only allow vodka
    case LAW_IMMIGRATION:
      return false; // Stalinists maintained tight border security at the Iron Curtain
    case LAW_ELECTIONS:
      return false; // Stalinists don't even have elections
    case LAW_MILITARY:
      return false; // Stalinists use the military for brutal repression
    case LAW_PRISONS:
      return false; // Stalinists annex Canada to fill it with gulags
    case LAW_TORTURE:
      return false; // Stalinists torture their enemies
    case LAWNUM:
      return false; // Liberals and Stalinists don't get along
    default:
      return false; // Liberals and Stalinists don't get along
    ...
    case VIEW_PRISONS:
      return false; // Stalinists annex Canada to fill it with gulags
    case VIEW_AMRADIO:
      return true; // Stalinists agree that Conservatives are bad
    case VIEW_CABLENEWS:
      return true; // Stalinists agree that Conservatives are bad
