...
int getsquad(int id)
{
  for (int i = 0; i < len(squad); i++)
    if (squad[i]->id == id) return i;
  return -1;
}
...
int getvehicletype(int id)
{
  for (int i = 0; i < len(vehicletype); i++)
    if (vehicletype[i]->id() == id) return i;
  return -1;
}
...
{
  for (int i = 0; i < len(vehicletype); i++)
    if (vehicletype[i]->idname() == idname) return i;
  return -1;
}
...
{
  for (int i = 0; i < len(cliptype); i++)
    if (cliptype[i]->get_id() == id) return i;
  return -1;
}
...
{
  for (int i = 0; i < len(cliptype); i++)
    if (cliptype[i]->get_idname() == idname) return i;
  return -1;
}
...
{
  for (int i = 0; i < len(weapontype); i++)
    if (weapontype[i]->get_id() == id) return i;
  return -1;
}
...
{
  for (int i = 0; i < len(weapontype); i++)
    if (weapontype[i]->get_idname() == idname) return i;
  return -1;
}
...
int getarmortype(int id)
{
  for (int i = 0; i < len(armortype); i++)
    if (armortype[i]->get_id() == id) return i;
  return -1;
}
...
{
  for (int i = 0; i < len(armortype); i++)
    if (armortype[i]->get_idname() == idname) return i;
  return -1;
}
...
int getloottype(int id)
{
  for (int i = 0; i < len(loottype); i++)
    if (loottype[i]->get_id() == id) return i;
  return -1;
}
...
{
  for (int i = 0; i < len(loottype); i++)
    if (loottype[i]->get_idname() == idname) return i;
  return -1;
}
...
{
  for (int i = 0; i < len(creaturetype); i++)
    if (crtype == creaturetype[i]->get_type()) return creaturetype[i];
  return NULL;
}
...
{
  for (int i = 0; i < len(creaturetype); i++)
    if (crtype == creaturetype[i]->get_idname()) return creaturetype[i];
  return NULL;
}
