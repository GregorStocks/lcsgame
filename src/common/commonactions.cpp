...
int randomissue(bool core_only)
{
  ...
  for (int i = 0; i < numviews; i++)
    if (roll < interest_array[i]) return i;
  return VIEW_MOOD;
}
...
{ // Use getsquad to treat members of a new squad being assembled as if not in a squad.
  ...
  bool a = ((first_in_squad && !second_in_squad)                                   //Squad member should come before squadless.
            || (first_in_squad && first->squadid < second->squadid)                //Older squads above newer.
            || (!first_in_squad && !second_in_squad && sort_name(first, second))); //Sort squadless by name.
              ...
              const string &producttype, const string &exitstring)
{
  ...
  while (true)
  {
    ...
    case 'O':
    case 'U':
      addstr("Press a Letter to select an " + producttype);
      break;
    default:
      addstr("Press a Letter to select a " + producttype);
      break;
    }
    move(23, 0);
    addpagestr();
...
int squadalive(const squadst *st)
{
  ...
  if (st)
    for (int p = 0; p < 6; p++)
      if (st->squad[p])
        if (st->squad[p]->alive) partyalive++;
  return partyalive;
}
