    ...
    for (int sq = 0; sq < len(squad); sq++) {
      ...
      if (squad[sq]->activity.type == ACTIVITY_VISIT) {
          ...
          for (int i = 0; i < 6; i++)
            if (squad[sq]->squad[i] && squad[sq]->squad[i]->carid != -1 && squad[sq]->squad[i]->is_driver) {
              ...
              case FIELDSKILLRATE_HARD:
                squad[sq]->squad[i]->train(SKILL_DRIVING, MAX(0, 5 - squad[sq]->squad[i]->get_skill(SKILL_DRIVING)));
                break;
              }
            }
        ...
        if (location[squad[sq]->activity.arg]->parent == travelLocation) {
          ...
          move(8, 1);
          int price;
          for (price = 0; price < 6; price++)
            if (!squad[sq]->squad[price]) break;
          price *= 100;
      ...
      if (clinictime(*pool[p])) {
        ...
        if (pool[p]->clinic == false) {
          ...
          for (int w = 0; w < BODYPARTNUM; w++) {
            ...
            // Non-bleeding wounds
            else { // Erase wound if almost fully healed, but preserve loss of limbs.
              if (pool[p]->blood >= 95)
                pool[p]->wound[w] &= WOUND_CLEANOFF;
            }
          }
  ...
  if (day == 3 && !disbanding)
    for (int l = 0; l < len(location); l++)
      if (location[l]->renting > 0 &&
          !location[l]->newrental) { // if rent >= 1000000 this means you get should kicked out automatically
  ...
  if (!disbanding)
    for (int r = len(recruit) - 1; r >= 0; r--) {
      ...
      if (p != -1 && ((pool[p]->location != -1 && location[pool[p]->location]->renting != RENTING_NOCONTROL && pool[p]->alive &&
                       location[pool[p]->location]->city == location[recruit[r]->recruit->location]->city) ||
...
void advancelocations() {
  ...
  for (int l = 0; l < len(location); l++) {
    if (location[l]->closed > 0) {
      location[l]->closed--;
      if (location[l]->closed == 0) { //Clean up graffiti, patch up walls, restore fire damage
    ...
    } else if (location[l]->highsecurity > 0) { // Bank will remain on high security much longer
...
int monthday() {
  switch (month) {
  case 2:
    return 28 + (year % 4 == 0 && (year % 100 != 0 || year % 400 == 0)); // February
  case 4:                                                                // April
  case 6:                                                                // June
  case 9:                                                                // September
  case 11:
