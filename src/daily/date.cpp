  ...
  if (aroll > troll) {
    ...
    if (loveslavesleft(*pool[p]) <= 0) {
      ...
      if (num_relationships == 1)
        addstr("someone!", gamelog);
      else
        addstr(tostring(num_relationships) + " people!", gamelog);
      gamelog.newline();
      move(y++, 0);
      addstr(pool[p]->name, gamelog);
      addstr(" isn't seductive enough to juggle ", gamelog);
      if (num_relationships == 1)
        addstr("another", gamelog);
      else
        addstr("yet another", gamelog);
    ...
    if (LCSrandom((aroll - troll) / 2) > d.date[e]->get_attribute(ATTRIBUTE_WISDOM, true)) {
      ...
      //Possibly date reveals map of location
        ...
        y++;
        move(y++, 0);
        addstr(s + d.date[e]->name + " turns the topic of discussion to the " + location[d.date[e]->worklocation]->name + ".", gamelog);
        gamelog.newline();
        move(y++, 0);
    ...
    switch (LCSrandom(4)) {
    case 0:
      addstr(s + " to wash " + d.date[e]->hisher() + " hair.", gamelog);
      break;
    case 1:
    ...
    case 2:
      ...
      break;
    case 3:
      addstr(s + " to catch " + d.date[e]->hisher() + " favourite TV show.", gamelog);
      break;
    case 4:
    ...
    case 5:
      ...
      break;
    case 6:
      addstr(s + " to recharge " + d.date[e]->hisher() + " cell phone.", gamelog);
      break;
    }
    move(y++, 0);
    addstr("They'll meet again tomorrow.", gamelog);
  ...
  for (e = len(d.date) - 1; e >= 0; e--) {
    ...
    while (true) {
      ...
      // Even liberals and moderates shouldn't be TOO easy to seduce! -- SlatersQuest
        ...
        troll += troll * (d.date[e]->juice / 200);
