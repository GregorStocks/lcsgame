...
void repairarmor(Creature &cr, char &clearformess)
{
      ...
      for (int l = 0; l < len(location[cr.location]->loot); l++)
        if (location[cr.location]->loot[l]->is_armor())
        {
          ...
          case 1: // Find something to clean if low skill, repair if high
            dothis = (a->is_bloody() && armor_makedifficulty(*a, &cr) > 4) || (a->is_damaged() && armor_makedifficulty(*a, &cr) <= 4);
            break;
  ...
  if (armor == NULL)
  {
    ...
    case 0:
      ...
      break;
    case 1:
      addstr(" reorganizes the armor closet.", gamelog);
      break;
    case 2:
      addstr(" cleans the kitchen.", gamelog);
      break;
    case 3:
    ...
    else
    {
      ...
      if (quality <= ((Armor *)it)->get_quality_levels())
      {
        ...
        case 4:
          ...
          break;
        default:
          addstr(quality, gamelog);
          addstr("th-rate", gamelog);
          break;
        }
...
void survey(Creature *cr)
{
  ...
  if (creatureskill < 1)
    noise = 18 + LCSrandom(3); // 18 to 20
  else if (creatureskill < 2)
    noise = 16 + LCSrandom(2); // 16 or 17
  else if (creatureskill < 3)
    noise = 14 + LCSrandom(2); // 14 or 15
  else if (creatureskill < 4)
    noise = 12 + LCSrandom(2); // 12 or 13
  else if (creatureskill < 5)
    noise = 10 + LCSrandom(2); // 10 or 11
  else if (creatureskill < 6)
    noise = 8 + LCSrandom(2); // 8 or 9
  else if (creatureskill < 7)
    noise = 7;
  else if (creatureskill < 9)
    noise = 6;
  else if (creatureskill < 11)
    noise = 5;
  else if (creatureskill < 14)
    noise = 4;
  else if (creatureskill < 18)
    noise = 3;
  else
    noise = 2;
  ...
  if (maxview != -1)
  {
    ...
    case VIEW_MILITARY:
      if (attitude[VIEW_MILITARY] > 50)
        addstr("military imperialism.");
      else
        addstr("defending the nation.");
      break;
...
bool stealcar(Creature &cr, char &clearformess)
{
  ...
  if (carselect(cr, cartype))
  {
    ...
    if (!cr.skill_check(SKILL_STREETSENSE, diff))
      do
        cartype = LCSrandom(len(vehicletype));
